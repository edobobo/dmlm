...
def pick_random_files(input_dir: str, n_files: int) -> List[str]:
    ...
    files_in_dir = [".".join(_f.split(".")[:-2]) for _f in files_in_dir if "data.xml" in _f]
...
def pick_files_and_convert_raganato_to_jsonl(
    ...
    for inventory in ["oxford", "wiktionary", "wordnet"]:
        random_files = pick_random_files(f"{input_dir}/{inventory}", chunks_per_inventory)
        selected_files += random_files
        corresponding_inventories += [inventory] * len(random_files)

    for key, group in groupby(zip(selected_files, corresponding_inventories), lambda x: x[1]):
        files_in_group = [x[0] for x in group]
        print(f"Inventory: {key} | ({len(files_in_group)}) Files: {', '.join(files_in_group)}")
